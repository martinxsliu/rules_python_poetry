...
from internal.pip_install.extract_wheels.lib import bazel, requirements
...
def main() -> None:
    ...
    parser.add_argument(
        "--requirement",
        action="store",
        help="Package requirement specifier.",
    )
    parser.add_argument(
        "--repo",
        action="store",
    ...
    single_package_mode = args.package is not None

    pip_args = [sys.executable, "-m", "pip", "--isolated", "wheel"]
    if args.requirements:
        pip_args += ["-r", args.requirements]
    elif args.package:
        package_requirement = args.package
        if args.requirement:
            package_requirement += args.requirement
        pip_args += ["--no-deps", package_requirement]
    ...
    targets = [
        ...
        % (
            args.repo,
            bazel.extract_wheel(
                whl, extras, pip_data_exclude, args.enable_implicit_namespace_pkgs, single_package_mode
            ),
        )
