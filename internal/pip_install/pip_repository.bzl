...
load("//internal/pip_install:repositories.bzl", "all_requirements")
...
def _pip_repository_impl(rctx):
    ...
    args = [
        python_interpreter,
        "-m",
        "internal.pip_install.extract_wheels",
        "--repo",
        "@%s" % rctx.attr.name,
    ]

    if rctx.attr.requirements:
        args += [
            "--requirements",
            rctx.path(rctx.attr.requirements),
        ]
    elif rctx.attr.package:
        args += [
            "--package",
            rctx.attr.package,
        ]
        if rctx.attr.requirement:
            args += [
                "--requirement",
                rctx.attr.requirement,
            ]
    else:
        fail("either 'requirements' or 'package' attribute must be specified")
...
"""),
        ...
        "requirement": attr.string(
            doc = "Package requirement specifier.",
        ),
